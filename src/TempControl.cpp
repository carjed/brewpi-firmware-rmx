	...
	case WAITING_FOR_PEAK_DETECT:
	{
		...
		{ // fridge temperature is too high
			...
			else
			{
				...
				{				  // If beer is already under target, stay/go to idle. 1/2 sensor bit idle zone
		...
		{ // fridge temperature is too low
			...
			{
				if (beerFast > (cs.beerSetting - 16))
				{				  // If beer is already over target, stay/go to idle. 1/2 sensor bit idle zone
...
void TempControl::setBeerTemp(temperature newTemp)
{
	...
	{			 // more than half degree C difference with old setting
